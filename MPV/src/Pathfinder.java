...
//Finds the shortest path between 2 points in a grid based map using 3 different algorithms.
public class Pathfinder {
	...
	public Pathfinder(){
		...
		openList2 = new LinkedList<Cell>();
		closedList = new ArrayList<Cell>();
	}

	//Finds the shortest path between 2 points in a grid.
	public ArrayList<Cell> findShortestPath(Cell start, Cell goal, Set<Cell> walls, Grid grid, int step, int algorithm){
		...
		switch(algorithm) {
			case BFS: {
				openList2.add(start);
				while (!openList2.isEmpty() && isRunning) {
					if (!isRunning)
						break;
					...
					if (timeSinceLastStep >= step) {
						...
						for (Edge e : current.getEdges()) {
							...
							if (next.getColor() != Color.GREEN && next.getColor() != Color.MAGENTA && next.getColor()!=Color.BLACK)
								next.setColor(Color.BLUE);

							//if we havent visited this cell yet, or we find a better path to it
							//then we add it to the openlist2 so it can be examined at a later time
							if (distanceFromStart < next.getDistanceFromStart() && next.getColor()!=Color.BLACK) {
								openList2.remove(next);
						...
						for (Edge e : current.getEdges()) {
							...
							switch (algorithm) {
								...
								case ASTAR: {
									...
									if (!openList.contains(next) || distanceFromStart < next.getDistanceFromStart() && next.getColor()!=Color.BLACK) {
										next.setDistanceFromStart(distanceFromStart);
